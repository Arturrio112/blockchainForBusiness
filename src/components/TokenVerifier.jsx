...
const TokenVerifier = ({ fractionalTokenAddress, nftAddress, tokenId }) => {
    ...
    const isNFTLocked =
        verification.nftOwner?.toLowerCase() ===
        contracts.fractionalization?.toLowerCase();
        ...
        <div className="mt-4 p-6 bg-gradient-to-br from-green-900/30 to-emerald-900/30 border-2 border-green-500 rounded-lg">
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-start space-x-3">
                    <div className="flex-1">
                        <p className="text-sm text-gray-400">
                            Your Fractional Tokens:
                        </p>
                        <p className="font-bold text-green-400">
                            {Number(verification.userBalance).toLocaleString()}{" "}
                            tokens
                        </p>
                        <p className="text-xs text-gray-500 mt-1">
                            Total Supply:{" "}
                            {Number(
                                verification.tokenTotalSupply
                            ).toLocaleString()}
                        </p>
                        <p className="text-xs text-purple-400 mt-1">
                            You own{" "}
                            {(
                                (Number(verification.userBalance) /
                                    Number(verification.tokenTotalSupply)) *
                                100
                            ).toFixed(4)}
                            % of the NFT
                        </p>
                ...
                <div className="pt-3 border-t border-gray-700">
                    <button
                        ...
                        className="cursor-pointer w-full px-4 py-2 rounded-lg bg-orange-600 hover:bg-orange-700 text-white font-bold text-sm transition-all border-2 border-orange-400"
                    >
                        Add Token to MetaMask
                    </button>
                    <p className="text-xs text-gray-400 mt-2 text-center">
                        Token: {verification.tokenName} (
                        {verification.tokenSymbol})
                    </p>
                </div>
            </div>
