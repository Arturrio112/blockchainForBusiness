...
const Fractionalize = () => {
  ...
  const handleMintNFT = async () => {
    ...
    try {
      ...
      const mockNFTContract = getMockNFTContract(contracts.mockNFT, signer);
  ...
  const handleFractionalize = async () => {
    ...
    if (!fractionalName.trim() || !fractionalSymbol.trim() || !totalSupply) {
    ...
    try {
      ...
      setSuccess(`Step 1/3: Approving NFT transfer... (Fee: $${latestFee.feeInUSD} = ${latestFee.feeInEth} ETH)`);
      ...
      // Fractionalize NFT
      setSuccess(`Step 2/3: Fractionalizing NFT... (Sending ${latestFee.feeInEth} ETH fee)`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-center mb-12">
          <div className={`flex items-center ${step >= 0 ? 'text-cyan-400' : 'text-gray-600'}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black border-2 ${
              step >= 0 ? 'bg-cyan-500 border-cyan-300 text-black' : 'border-gray-600'
            }`}>
              1
            </div>
            <span className="ml-3 font-bold text-sm">Select NFT</span>
          </div>
          
          <div className={`w-16 h-1 mx-2 ${step >= 1 ? 'bg-cyan-500' : 'bg-gray-700'}`}></div>
          
          <div className={`flex items-center ${step >= 1 ? 'text-cyan-400' : 'text-gray-600'}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black border-2 ${
              step >= 1 ? 'bg-cyan-500 border-cyan-300 text-black' : 'border-gray-600'
            }`}>
              2
            </div>
            <span className="ml-3 font-bold text-sm">Mint/Load</span>
          </div>
          
          <div className={`w-16 h-1 mx-2 ${step >= 2 ? 'bg-cyan-500' : 'bg-gray-700'}`}></div>
          
          <div className={`flex items-center ${step >= 2 ? 'text-cyan-400' : 'text-gray-600'}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black border-2 ${
              step >= 2 ? 'bg-cyan-500 border-cyan-300 text-black' : 'border-gray-600'
            }`}>
              3
            </div>
            <span className="ml-3 font-bold text-sm">Fractionalize</span>
          </div>
        </div>
        ...
        {step === 0 && (
          <div className="bg-gradient-to-br from-purple-900/50 to-indigo-900/50 backdrop-blur-sm border-2 border-cyan-500 rounded-2xl p-8 shadow-2xl">
            <h2 className="text-2xl font-black text-cyan-400 mb-6">STEP 1: SELECT YOUR NFT</h2>
            
            {loadingNFTs ? (
              <div className="text-center py-12">
                <p className="text-gray-300 animate-pulse">Loading your NFTs...</p>
              </div>
            ) : ownedNFTs.length > 0 ? (
              <div>
                <p className="text-gray-300 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  {ownedNFTs.map((nft) => (
                    <div
                      key={nft.tokenId}
                      onClick={() => {
                        setNftTokenId(nft.tokenId.toString());
                        setStep(2);
                      }}
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <div className="flex-1">
                          <p className="font-bold text-cyan-400">Token #{nft.tokenId}</p>
                          <p className="text-xs text-gray-400 truncate">{nft.uri}</p>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        {step === 1 && (
          <div className="bg-gradient-to-br from-purple-900/50 to-indigo-900/50 backdrop-blur-sm border-2 border-cyan-500 rounded-2xl p-8 shadow-2xl">
            <h2 className="text-2xl font-black text-cyan-400 mb-6">STEP 1: MINT A TEST NFT</h2>
            <p className="text-gray-300 mb-6">
              For testing purposes, mint a mock NFT that you can then fractionalize.
            </p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Enter any URI or text for testing (e.g., "My Test NFT #1")
                </p>
              </div>
              ...
              <div className="text-center pt-4 border-t border-gray-700">
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    onClick={() => {
                      if (nftTokenId) {
                        setSuccess(`Using Token ID: ${nftTokenId}`);
                        setStep(2);
                      } else {
        ...
        {step === 2 && (
          <div className="bg-gradient-to-br from-purple-900/50 to-indigo-900/50 backdrop-blur-sm border-2 border-cyan-500 rounded-2xl p-8 shadow-2xl">
            <h2 className="text-2xl font-black text-cyan-400 mb-6">STEP 2: FRACTIONALIZE NFT</h2>
            
            <div className="mb-6 p-4 bg-green-900/30 border-2 border-green-500 rounded-lg">
              <p className="text-green-300 font-semibold">
                NFT Token ID: <span className="text-cyan-400">#{nftTokenId}</span>
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFractionalName(e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFractionalSymbol(e.target.value.toUpperCase())}
              ...
              {loadingFee ? (
                <div className="p-4 bg-purple-900/30 border-2 border-purple-500 rounded-lg text-center">
                  <div className="animate-pulse">
                    <span className="text-gray-400">Fetching current ETH price...</span>
                  </div>
                </div>
              ) : feeInfo ? (
                <div className={`p-4 bg-gradient-to-r ${feeInfo.usingFallback ? 'from-yellow-900/30 to-orange-900/30 border-yellow-500' : 'from-green-900/30 to-emerald-900/30 border-green-500'} border-2 rounded-lg`}>
                  <h3 className={`text-sm font-bold ${feeInfo.usingFallback ? 'text-yellow-400' : 'text-green-400'} mb-3 uppercase`}>
                    Platform Fee {feeInfo.usingFallback && '(Fallback Mode)'}
                  </h3>
                  
                  {feeInfo.usingFallback && (
                    <div className="mb-3 p-2 bg-yellow-900/30 border border-yellow-600 rounded text-xs text-yellow-300">
                      Warning: Unable to fetch live price. Using fixed 0.002 ETH fee.
                    </div>
                  )}
                  
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-300 text-sm">Fixed Fee (USD):</span>
                      <span className={`font-black text-lg ${feeInfo.usingFallback ? 'text-yellow-400' : 'text-green-400'}`}>
                        ${feeInfo.feeInUSD}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-300 text-sm">
                        {feeInfo.usingFallback ? 'Est. ETH Price:' : 'Current ETH Price:'}
                      </span>
                      <span className="text-cyan-400 font-bold">
                        ${Number(feeInfo.ethPriceUSD).toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center pt-2 border-t border-gray-700">
                      <span className="text-gray-300 text-sm">You will pay:</span>
                      <span className={`font-black text-lg ${feeInfo.usingFallback ? 'text-orange-400' : 'text-yellow-400'}`}>
                  ...
                  <button
                    ...
                    className={`mt-3 w-full px-3 py-2 text-xs rounded-lg ${feeInfo.usingFallback ? 'bg-yellow-800 hover:bg-yellow-700' : 'bg-purple-800 hover:bg-purple-700'} text-white font-bold transition-all`}
                  >
                    {feeInfo.usingFallback ? 'Retry Fetching Price' : 'Refresh Price'}
                  </button>
                </div>
              ) : (
                <div className="p-4 bg-purple-900/30 border-2 border-purple-500 rounded-lg text-center">
                  <span className="text-gray-400 text-sm">Loading fee information...</span>
                </div>
              )}
              ...
              {totalSupply && (
                <div className="p-4 bg-purple-900/30 border-2 border-purple-500 rounded-lg">
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    All fractional tokens go to your wallet (no fee deduction from supply)
                  </p>
                </div>
              )}
              ...
              <div className="flex space-x-4">
                ...
                <button
                  ...
                  disabled={isLoading || !fractionalName.trim() || !fractionalSymbol.trim() || !totalSupply}
